...
ADMIN_ID = 6759191586
API_TOKEN = "cb31934b7ce14c3fb458ae7ba0882d24"
...
def admin_menu():
    ...
    markup.add(
        types.InlineKeyboardButton("➕ إضافة نقاط", callback_data="admin_add"),
        types.InlineKeyboardButton("➖ خصم نقاط", callback_data="admin_sub")
    )
    markup.add(
        types.InlineKeyboardButton("🚫 حظر مستخدم", callback_data="admin_ban"),
        types.InlineKeyboardButton("✅ فك الحظر", callback_data="admin_unban")
    )
    markup.add(
        types.InlineKeyboardButton("💰 سعر البطاقة", callback_data="admin_price"),
...
def admin_panel(message):
    if message.from_user.id != ADMIN_ID:
        return
    bot.reply_to(message, "👑 **لوحة تحكم الإدارة**\n\nاختر الإجراء الذي تريده من الأزرار أدناه:", reply_markup=admin_menu(), parse_mode="Markdown")
...
def admin_callbacks(call):
    ...
    if call.data == "admin_stats":
        ...
        msg = bot.send_message(ADMIN_ID, "➕ أرسل أيدي (ID) المستخدم الذي تريد إضافة نقاط له:")
        bot.register_next_step_handler(msg, step_add_id)
        
    elif call.data == "admin_sub":
        msg = bot.send_message(ADMIN_ID, "➖ أرسل أيدي (ID) المستخدم الذي تريد خصم نقاط منه:")
        bot.register_next_step_handler(msg, step_sub_id)
        
    elif call.data == "admin_ban":
        msg = bot.send_message(ADMIN_ID, "🚫 أرسل أيدي (ID) المستخدم الذي تريد حظره:")
        bot.register_next_step_handler(msg, step_ban_id)
        
    elif call.data == "admin_unban":
        msg = bot.send_message(ADMIN_ID, "✅ أرسل أيدي (ID) المستخدم لفك الحظر عنه:")
        bot.register_next_step_handler(msg, step_unban_id)
        
    elif call.data == "admin_price":
        msg = bot.send_message(ADMIN_ID, "💰 أرسل السعر الجديد للبطاقة (بالنقاط):")
        bot.register_next_step_handler(msg, step_set_price)

# دوال المعالجة المتسلسلة (الأزرار)
def step_add_id(message):
    try:
        target_id = int(message.text)
        msg = bot.send_message(ADMIN_ID, "كم نقطة تريد إضافتها؟")
        bot.register_next_step_handler(msg, step_add_amount, target_id)
    except:
        bot.send_message(ADMIN_ID, "❌ الأيدي غير صحيح، يجب أن يكون أرقاماً فقط. افتح اللوحة مجدداً.")

def step_add_amount(message, target_id):
    try:
        ...
        conn.commit()
        conn.close()
        bot.send_message(ADMIN_ID, f"✅ تم إضافة {amount} نقطة للمستخدم {target_id} بنجاح.")
        try:
            bot.send_message(target_id, f"🎁 إضافة إدارية: تم إضافة {amount} نقطة لرصيدك!")
        except:
            pass
    except:
        bot.send_message(ADMIN_ID, "❌ النقاط يجب أن تكون أرقاماً فقط.")

def step_sub_id(message):
    try:
        target_id = int(message.text)
        msg = bot.send_message(ADMIN_ID, "كم نقطة تريد خصمها؟")
        bot.register_next_step_handler(msg, step_sub_amount, target_id)
    except:
        bot.send_message(ADMIN_ID, "❌ الأيدي غير صحيح.")

def step_sub_amount(message, target_id):
    try:
        amount = int(message.text)
        conn = sqlite3.connect('bot_database.db')
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        bot.send_message(ADMIN_ID, f"✅ تم خصم {amount} نقطة من المستخدم {target_id} بنجاح.")
    except:
        bot.send_message(ADMIN_ID, "❌ النقاط يجب أن تكون أرقاماً فقط.")

def step_ban_id(message):
    try:
        target_id = int(message.text)
        conn = sqlite3.connect('bot_database.db')
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        bot.send_message(ADMIN_ID, f"✅ تم حظر المستخدم {target_id} بنجاح.")
    except:
        bot.send_message(ADMIN_ID, "❌ الأيدي غير صحيح.")

def step_unban_id(message):
    try:
        target_id = int(message.text)
        conn = sqlite3.connect('bot_database.db')
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        bot.send_message(ADMIN_ID, f"✅ تم فك الحظر عن المستخدم {target_id} بنجاح.")
    except:
        bot.send_message(ADMIN_ID, "❌ الأيدي غير صحيح.")
...
def step_set_price(message):
    try:
        ...
        conn = sqlite3.connect('bot_database.db')
        c = conn.cursor()
        c.execute("UPDATE settings SET value = ? WHERE key = 'card_price'", (price,))
        conn.commit()
        conn.close()
        bot.send_message(ADMIN_ID, f"✅ تم تغيير سعر البطاقة في المتجر إلى {price} نقطة.")
    except:
        bot.send_message(ADMIN_ID, "❌ السعر يجب أن يكون أرقاماً فقط.")
...
def handle_text(message):
    ...
    if text == "💰 رصيدي":
        ...
        btn = types.InlineKeyboardButton(f"💳 شراء بطاقة (السعر: {price} نقطة)", callback_data="buy_test_card")
        markup.add(btn)
